...
const analysis = {
  ...
  analyzeSamples: (script, report, defaultErrorBudget) => {
    ...
    const mergeReportPropertyCounts = (property) => reports.forEach((test) => Object.keys(test[property]).forEach((key) => {
    ...
    const accumulateReportTotals = (property) => reports.forEach((test) => { totals[property] += test[property] })
   ...
   * @returns Object
   */
  analyzeAcceptance: (timeNow, script, settings, results) => analysis.analyzeSamples(script, results, sampling.defaults.acceptance.errorBudget),
   ...
   * @returns Object
   */
  analyzeMonitoring: (timeNow, script, settings, results) => analysis.analyzeSamples(script, results, sampling.defaults.monitoring.errorBudget),
