...
const RunCommand = require('artillery/lib/cmds/run') // eslint-disable-line import/no-unresolved
...
const artilleryTask = {
  ...
  execute: (timeNow, payload) => new Promise((resolve, reject) => {
    ...
    const args = ['--output', output, '--input', scriptPath]

    if (environment) {
      args.push('--environment', environment)
    }

    args.push('emptyDummyScript.yml')
