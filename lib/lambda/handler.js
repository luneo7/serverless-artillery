...
const task = require('./artillery-task')
const platform = require('./platform-settings')
...
const lambdaHandler = {
  ...
  mergeIf: (script, readMergeFile = lambdaHandler.readMergeFile) => (mergeFileField in script
    ? readMergeFile(script[mergeFileField])
      .then((inputData) => merge({}, inputData, omit(script, [mergeFileField])))
    : Promise.resolve(script)),

  createHandler: (taskToExecute, platformSettings) => (event, context, callback) => {
    try {
      const script = event
      script._funcAws = {
