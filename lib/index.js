...
const resolveInput = require('serverless/lib/cli/resolve-input')
...
const resolveVariables = require('serverless/lib/configuration/variables/resolve')
const filterSupportedOptions = require('serverless/lib/cli/filter-supported-options')

const Serverless = require('serverless')
const npm = require('./npm')

const platformSettings = require('./lambda/platform-settings')
const modes = require('./lambda/modes')
const planning = require('./lambda/planning')

const constants = {
  CompatibleServerlessSemver: '^3.39.0',
...
const impl = {
  ...
  getServerless: async () => {
    const serviceDir = impl.findServicePath()
    // eslint-disable-next-line global-require
    const { commands, options, commandSchema } = resolveInput(require('serverless/lib/cli/commands-schema/aws-service'))
    const configuration = await readConfiguration(path.join(serviceDir, 'serverless.yml'))
    ...
    const resolverConfiguration = {
      ...
      variablesMeta,
      sources: {
        env: require('serverless/lib/configuration/variables/sources/env'), // eslint-disable-line global-require
        file: require('serverless/lib/configuration/variables/sources/file'), // eslint-disable-line global-require
        opt: require('serverless/lib/configuration/variables/sources/opt'), // eslint-disable-line global-require
        self: require('serverless/lib/configuration/variables/sources/self'), // eslint-disable-line global-require
        strToBool: require('serverless/lib/configuration/variables/sources/str-to-bool'), // eslint-disable-line global-require
        sls: require('serverless/lib/configuration/variables/sources/instance-dependent/get-sls')(), // eslint-disable-line global-require
      },
      options: filterSupportedOptions(options, { commandSchema, providerName }),
      fulfilledSources: new Set(['file', 'self', 'strToBool']),
      propertyPathsToResolve: null,
   ...
   */
  parseScript: (input) => yaml.load(input),
  ...
  /**
   * Generates a new script to be used with environment config
   * If an environment is used it will try to use the phases in the environment definition,
   * if the environment doesn't have any phases object it will default to the global phases
   * and will filter out any other environment that is not being run at the time
   * @param script The script to obtain a new script based in the environment config.
   * @returns {object} A new script taking into account environment config.
   */
  generateEnvironmentScript: (script) => {
    let { phases } = script.config
    const { environment, config: { environments } } = script
    let filteredEnvironments

    if (environment && environments && environments[environment]) {
      if (environments[environment].phases) {
        ({ phases } = environments[environment])
      }
      const { phases: _phases, ...envConfig } = environments[environment]
      filteredEnvironments = { [environment]: { ...envConfig } }
    }
    ...
    return {
      ...
      config: {
        ...script.config,
        phases,
        ...(filteredEnvironments && { environments: filteredEnvironments }),
      },
    }
  },
  ...
  scriptConstraints: (script) => {
    ...
    const httpTimeout = 120 // default AWS setting
    // if (
    //   aws.config &&
...
`,
   ...
   */
  serverlessRunner: async (options) => {
    // proceed with using SLS
    const serverless = await impl.getServerless()
    if (options.verbose) {
    ...
    let result
    // add our intercepter
    const invoke = serverless.pluginManager.plugins[serverless.pluginManager.plugins.findIndex(
  ...
  */
  serverlessLoader: async () => impl.getServerless()
    .then((serverless) => {
  ...
  invoke: (options) => impl.getScriptText(options)
    ...
    .then((input) => {
      ...
      if (options.e || options.environment) {
        ...
        script = impl.generateEnvironmentScript(script)
      }
      ...
      const environmentMessage = script && script.environment ? ` with environment: ${script.environment}` : ''
      logIf(`${os.EOL}\tInvoking test Lambda${environmentMessage}${os.EOL}`)
  ...
  kill: (options) => {
    ...
    return impl.serverlessLoader()
      .then((serverless) => {
        ...
        return lambda.putFunctionConcurrency(params)
          .catch((err) => {
            if (options.debug) {
            ...
            if (err.name === 'ResourceNotFoundException') {
              ...
              throw new Error(`${os.EOL}Unexpected error setting concurrency to zero: ${err.message} ${err.name}${os.EOL}`)
            }
          })
      })
      .then(() => {
