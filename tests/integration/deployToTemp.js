...
const execError = (err, stderr) => new Error(`${err.message} ${stderr}`)

const namesToFullPaths = (directory) => (names) => names.map((name) => join(directory, name))

const filterOutSpecFiles = (names) => names.filter((name) => !name.endsWith('.spec.js'))
...
const impl = {
  findTargetSourceFiles: (ls = fs.ls, sourcePath = defaultTargetSourcePath) => () => ls(sourcePath)
  ...
  stageTarget: (
    ...
    copyAll = fs.copyAll
  ) => (destination) => findTargetSourceFiles()
  ...
  stageSlsart: (
    ...
    copyAll = fs.copyAll,
    exec = impl.execAsync()
  ) => (destination) => findSlsartSourceFiles()
    .then(copyAll(destination))
    .then(() => exec('npm i', { cwd: destination })),

  execAsync: (exec = childProcess.exec,
    log = defaultLog,
    warn = defaultWarn) => (command, options = {}) => new Promise((resolve, reject) => {
    exec(command, options, (err, stdout, stderr) => (err
      ? warn('execAsync ERROR: ', err, stderr, stdout) || reject(execError(err, stderr))
      : log('execAsync SUCCESS: ', stdout) || resolve(stdout)))
  }),

  deploy: (exec = impl.execAsync()) => (directory) => (process.env.DEBUG ? exec('sls deploy -v', { cwd: directory }) : exec('sls deploy', { cwd: directory })),

  tempLocation: (random = () => `${Date.now()}`, root = defaultRoot) => (instanceId = random()) => ({ instanceId, destination: join(root, instanceId) }),
  ...
  deployNewTestResources: (
    ...
    return mkdirp(slsartTempFolder)
      ...
      .then((urls) => ({ ...urls, ...paths }))
      .then(tap(log))
      .catch((err) => warn('failed to deploy a new target:', err.stack) || false)
  },

  remove: (exec = impl.execAsync()) => (directory) => exec('sls remove', { cwd: directory }),
  ...
  listTempDeployments: (ls = fs.ls) => (root) => ls(root)
    .then((directories) => directories.map((directory) => join(root, directory))),
